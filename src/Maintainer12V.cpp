...
void Maintainer12V::SetInitByMaintainer(bool initbyM) {
  Maintainer12V::initbyMaintain = initbyM;
}

bool Maintainer12V::GetInitByMaintainer() { return Maintainer12V::initbyMaintain; }

bool Maintainer12V::GetRunMaintainer() { return Maintainer12V::runMaintainer; }

void Maintainer12V::Ms10Task() {
  Param::SetInt(Param::opmode, MOD_MAINTAIN);
  ErrorMessage::UnpostAll();
  if (!Maintainer12V::runMaintainer) {
...
void Maintainer12V::ParamsChange() {
  maintainTicks =
      (GetInt(Param::wakeupMin) *
       300); // number of 200ms ticks that equates to maintaince timer in minutes
  maintainDur_tmp = GetInt(Param::wakeupMin);
}